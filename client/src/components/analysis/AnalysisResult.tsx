...
import { CheckCircle2, AlertCircle, AlertTriangle, HelpCircle, Download, TrendingDown, Shield, Calendar, ExternalLink, Share2, Copy } from 'lucide-react';
...
export function AnalysisResult({ data }: AnalysisResultProps) {
  ...
  const handleDownload = async () => {
    ...
    const buttons = reportRef.current.querySelector('.action-buttons') as HTMLElement;
    ...
    try {
      ...
      pdf.save(`JustBill-Analysis-${hospitalName.replace(/\s+/g, '-')}.pdf`);
    } catch (err) {
      console.error('Download failed:', err);
      alert('PDF generation failed. Please try again.');
    } finally {
      if (buttons) buttons.style.display = originalDisplay || 'flex';
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'JustBill Analysis Result',
      text: `JustBill Analysis for ${hospitalName}: Potentially overcharged by ₹${summary.overchargeAmount}. Check details!`,
      url: window.location.href, // Looking to share the current app context if possible, or just text.
    };
  ...
  return (
    <>
      ...
      <div ref={reportRef} className="w-full max-w-5xl mx-auto space-y-6 relative p-4 bg-white/50 backdrop-blur-sm rounded-xl">
        {/* Background for PDF capture consistency */}
        ...
        <motion.div
          ...
          className="flex gap-3 justify-center action-buttons no-print"
        >
          <button
            onClick={handleDownload}
            disabled={isDownloading}
            className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-lg text-sm font-medium transition-all duration-200 shadow-md hover:shadow-lg cursor-pointer disabled:opacity-70"
          >
            {isDownloading ? (
          ...
          <button
            onClick={handleShare}
            className="flex items-center gap-2 px-6 py-3 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-sm font-medium transition-colors cursor-pointer"
          >
